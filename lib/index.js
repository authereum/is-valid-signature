...
const isValidSignature = async (provider, signerAddress, message, signature) => {
  if (!(typeof provider === 'object' || typeof provider === 'string')) {
    throw new Error('Expected web3 or provider object')
  }
  ...
  if (provider.connection && provider.connection.url) {
    provider = new Web3.providers.HttpProvider(provider.connection.url)
  }

  // url
  if (typeof provider === 'string' && provider.indexOf('http') === 0) {
